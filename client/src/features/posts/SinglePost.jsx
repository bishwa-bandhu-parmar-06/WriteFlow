    ...
    {(currentUser?._id === userId?._id?.toString() || 
      ...
      <>
        ...
        <button 
          ...
          disabled={isDeleting}
          className={`block px-4 py-2 text-sm text-red-600 hover:bg-gray-100 w-full text-left ${isDeleting ? 'opacity-50' : ''}`}
        >
      ...
      <div className="flex justify-between text-gray-600 text-sm border-t pt-3">
        <button 
          onClick={handleLike}
          className={`flex items-center hover:text-indigo-600 transition duration-150 ${isLiked ? 'text-indigo-600' : ''}`}
        ...
        <div className="mt-4 border-t pt-3">
          {currentUser ? (
            ...
            <p className="text-sm text-gray-500 mb-3">
              Please <button onClick={() => navigate('/auth')} className="text-indigo-600 hover:underline">login</button> to comment
            </p>
          )}
